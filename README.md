...
Open `http://localhost:8080/` after `docker compose up -d` to load the bundled `index.html` through Nginx. The page proxies Martin style and `layer_*` tile requests through the same origin, so you do not need to edit host/IP values in the HTML anymore.